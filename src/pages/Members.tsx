  ...
  const handleAction = async () => {
    ...
    try {
      ...
      setMembers(members.map(m => 
        m.id === selectedMember.id ? { ...m, status: actionType === 'approve' ? 'approved' : 'rejected' } : m
      ));
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-2">
        ...
        <p className="text-slate-500">Review and manage participation for this Equb circle.</p>
      </div>
      ...
      <Modal 
        ...
        onClose={() => { setSelectedMember(null); setActionType(null); }}
          ...
          <>
            ...
            <button 
              ...
              className={`rounded-xl px-6 py-2 text-sm font-bold text-white transition-all shadow-lg active:scale-95 ${
                actionType === 'approve' ? 'bg-emerald-500 shadow-emerald-200' : 'bg-red-500 shadow-red-200'
              }`}
            >
              Confirm {actionType}
            </button>
