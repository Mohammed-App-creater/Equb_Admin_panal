          ...
          notifications.map(n => (
            <div 
              ...
              className={`group flex items-start gap-4 p-5 rounded-3xl border transition-all ${
                n.is_read ? 'bg-card border-border' : 'bg-primary/5 border-primary/10 ring-1 ring-primary/5'
              }`}
            >
              <div className={`h-10 w-10 shrink-0 rounded-2xl flex items-center justify-center ${
                n.notif_type === 'alert' ? 'bg-destructive/10 text-destructive' : 
                n.notif_type === 'success' ? 'bg-emerald-500/10 text-emerald-500' : 'bg-primary/10 text-primary'
              }`}>
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start mb-1">
                  <h4 className={`text-sm font-bold ${n.is_read ? 'text-foreground' : 'text-primary'}`}>{n.notif_type}</h4>
                  <span className="text-[10px] font-bold text-muted-foreground uppercase">{n.updated_at}</span>
                </div>
